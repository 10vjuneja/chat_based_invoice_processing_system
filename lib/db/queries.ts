...
export async function saveInvoiceLineItem({
    id,
    invoiceId,
    description,
    quantity,
    ...
    try {
        return await db.insert(invoiceLineItem).values({
            id,
            invoiceId,
            description,
            quantity,
        ...
        console.error('Failed to save invoice line item in database');
        throw error;
    }
}
...
export async function getInvoiceLineItems({ invoiceId }: { invoiceId: string }) {
    try {
        return await db
            .select()
...
export async function getLastNInvoices(num_invoices: number = 10) {
    try {
        return await db
            .select()
            .from(invoice)
        ...
        console.error('Failed to get last 10 invoices from database');
        throw error;
    }
}
...
export async function saveTokenUsage({
    ...
    try {
        return await db.insert(invoiceTokenUsage).values({
            id,
            invoiceId,
            model,
            promptTokens,
...
export async function getTokenUsageByInvoiceId({ invoiceId }: { invoiceId: string }) {
    try {
        return await db
            .select()
            .from(invoiceTokenUsage)
            .where(eq(invoiceTokenUsage.invoiceId, invoiceId));
    } catch (error) {
        console.error('Failed to get token usage by invoice id from database');
        throw error;
    }
}
...
export async function getAverageTokenUsage() {
    try {
        const result = await db
            .select({
                avgPromptTokens: sql<number>`avg(promptTokens)`,
                avgCompletionTokens: sql<number>`avg(completionTokens)`,
                avgTotalTokens: sql<number>`avg(totalTokens)`,
        ...
        console.error('Failed to get average token usage from database');
        throw error;
    }
}
